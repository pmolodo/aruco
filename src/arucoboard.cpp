...
Mat createBoard( Size gridSize,int MarkerSize,int MarkerDistance,unsigned int FirstMarkerID, BoardConfiguration& TInfo  ) throw (cv::Exception)
{
  ...
  vector<vector<int> > *TheMarkersIds=NULL; 
	...
	unsigned int idp=0;
	for(unsigned int i=0;i<gridSize.height;i++)
		for(unsigned int j=0;j<gridSize.width;j++,idp++)
 ...
 for(int y=0;y<gridSize.height;y++)
	 for(int x=0;x<gridSize.width;x++){
		...
		Mat marker=createMarker( TInfo._markersId.at<int>(y,x),MarkerSize);
...
BoardConfiguration::BoardConfiguration(const BoardConfiguration  &T)
{
	...
	*/
	void BoardConfiguration::saveToFile(string sfile)throw (cv::Exception)
	{
	  ...
	  for(unsigned int i=0;i<_markersId.size().height;i++){
	    for(unsigned int j=0;j<_markersId.size().width;j++)
	...
	*
	*/
float ArBoardDetector::detect(const vector<Marker> &detectedMarkers,const  BoardConfiguration &BConf, Board &Bdetected, Mat camMatrix,Mat distCoeff,float markerSizeMeters)throw (cv::Exception)
{
// cout<<"markerSizeMeters="<<markerSizeMeters<<endl;
  ...
  for(unsigned int i=0;i<detectedMarkers.size();i++){
    ...
    //find it
     for(unsigned int j=0;j<detected.size().height && ! found;j++)
       for(unsigned int k=0;k<detected.size().width && ! found;k++)
...
if (camMatrix.rows!=0 && markerSizeMeters>0 && detectedMarkers.size()>1){
  ...
  int currIndex=0;
  for(unsigned int y=0;y<detected.size().height;y++)
     for(unsigned int x=0;x<detected.size().width;x++){
	if (  detected.at<int>(y,x)!=-1 ){
		...
		//tranaltion to make the Ref System be in center
	      ...
	      objPoints.at<float>( currIndex,0)= AX;objPoints.at<float>( currIndex,1)= AY;objPoints.at<float>( currIndex,2)= 0;
	      objPoints.at<float>( currIndex+1,0)= AX;objPoints.at<float>( currIndex+1,1)= AY+markerSizeMeters;objPoints.at<float>( currIndex+1,2)= 0;
	      objPoints.at<float>( currIndex+2,0)= AX+markerSizeMeters;objPoints.at<float>( currIndex+2,1)= AY+markerSizeMeters;objPoints.at<float>( currIndex+2,2)= 0;
	      objPoints.at<float>( currIndex+3,0)= AX+markerSizeMeters;objPoints.at<float>( currIndex+3,1)= AY;objPoints.at<float>( currIndex+3,2)= 0;
	      currIndex+=4;
	}
  }
...
void ArBoardDetector::rotateXAxis(Mat &rotation)
{
